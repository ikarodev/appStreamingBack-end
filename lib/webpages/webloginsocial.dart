...
class _WebLoginSocialState extends State<WebLoginSocial> {
  ...
  Future<void> _gmailLogin() async {
    try {
      // Configuração específica para web com clientId explícito
      final GoogleSignIn googleSignIn = GoogleSignIn(
        clientId: '1065762459264-95qe1152pqgcgpidk002072vqci8fgi8.apps.googleusercontent.com',
        scopes: ['email', 'profile', 'openid'],
        // Remover configurações que podem causar redirect issues
      );
      
      // Para web, primeiro tente fazer logout para limpar estado
      try {
        await googleSignIn.signOut();
      } catch (e) {
        printLog('Logout silencioso falhou: $e');
      }
      
      // Use signIn diretamente para web (mais confiável que signInSilently)
      GoogleSignInAccount? googleUser = await googleSignIn.signIn();
      
      if (googleUser == null) {
        printLog('Usuário cancelou o login ou não conseguiu fazer login');
        return;
      }
      ...
      // Verificar se temos informações básicas necessárias
      if (user.email == null || user.email!.isEmpty) {
        printLog('Email do usuário não disponível');
        Utils.hideProgress();
        Utils.showToast('Não foi possível obter o email do usuário');
        return;
      }

      // Para web, não precisamos do Firebase Auth, vamos direto para a API
      printLog("Processando login com informações do Google...");
      
      String userName = user.displayName ?? user.email!.split('@')[0];
      String userEmail = user.email!;
      
      printLog("UserName processado: $userName");
      printLog("UserEmail processado: $userEmail");
      ...
      await checkAndNavigate(userEmail, userName, "2");
      ...
      if (errorMessage.contains('NetworkError') || errorMessage.contains('FedCM')) {
        Utils.showToast('Erro de rede no Google Sign-In. Tente novamente ou verifique sua conexão.');
      } else if (errorMessage.contains('popup_closed_by_user')) {
        Utils.showToast('Login cancelado pelo usuário.');
      } else if (errorMessage.contains('access_denied')) {
        Utils.showToast('Acesso negado. Verifique as permissões do Google.');
      } else {
        Utils.showToast('Erro inesperado no Google Sign-In. Tente novamente.');
      }
    }
  }
  ...
  checkAndNavigate(String mail, String displayName, String type) async {
    ...
    printLog('Chamando loginWithSocial API...');
    try {
      ...
      printLog('API Response Status: ${generalProvider.loginSocialModel.status}');
      printLog('API Response Message: ${generalProvider.loginSocialModel.message}');
    } catch (e) {
