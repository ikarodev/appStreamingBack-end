...
import 'package:handsplay/model/qualitymodel.dart';
import 'package:handsplay/model/sharemodel.dart';
import 'package:handsplay/model/subtitlemodel.dart';
import 'package:handsplay/pages/bottombar.dart';
import 'package:handsplay/pages/loginsocial.dart';
...
import 'package:handsplay/provider/mysubscribedplanprovider.dart';
import 'package:handsplay/provider/playerprovider.dart';
import 'package:handsplay/provider/profileprovider.dart';
import 'package:handsplay/provider/showdetailsprovider.dart';
import 'package:handsplay/provider/subscriptionprovider.dart';
import 'package:handsplay/provider/videodetailsprovider.dart';
...
import 'package:handsplay/webwidget/webdialogs.dart';
...
class Utils {
  ...
  static Future<dynamic> openSubscription({
    ...
    if (profileProvider.profileModel.result != null &&
      ...
      if (!context.mounted) return;
      if (kIsWeb) {
        isSubscribe = await context.pushNamed(
  ...
  static openShareDialog({
    ...
    String appendParams =
        "newpage=${shareModel.newPage}&videoid=${shareModel.videoId}&typeid=${shareModel.typeId}&videotype=${shareModel.videoType}&subvideotype=${shareModel.subVideoType}";
    String encodedParams = base64Encode(utf8.encode(appendParams));
    printLog("appendParams ======> $appendParams");
    printLog("encodedParams =====> $encodedParams");
    printLog("deeplinkDomain ====> ${Constant.deeplinkDomain}");

    String contentLink = "https://${Constant.deeplinkDomain}/?$encodedParams";
  ...
  static Widget progressWithPercentage(progress) {
    return Align(
      alignment: Alignment.center,
